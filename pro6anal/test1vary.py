...
class1 = np.clip(np.round(class1_adj).astype(int), 10, 100).astype(int)
class2 = np.clip(np.round(class2_adj).astype(int), 10, 100).astype(int)
print(class1[:10])
print(class2[:10])
...
print(df.head())
df.to_csv('test1vary.csv', index=False, encoding='utf-8-sig')
...
plt.hlines(
    target_mean, 0.5, 2.5, colors='red', 
...
# 대립가설: 두 반의 국어점수의 표준편차(평균)는 차이가 있다.
