...
try:
    ...
    with open('ftext3.txt', mode = 'w', encoding='utf-8') as fobj2:
        print(fobj2.read())
        
except Exception as e:
    print('err: ',e)

print('\n\n피클링(일반 객체 및 복합 객체 처리)')
import pickle
...
try:
    ...
    with open('hello.dat', mode='wb') as fobj3:     # mode = 'wb' >> binary 형태로 저장하기
    ...
    with open("hello.dat", mode = 'rb') as fobj4:
        a, b = pickle.load(fobj4)                   # pickle.load(파일 객체)
        print(a)
        print(b)
        c = pickle.load(fobj4)                      #
        print('c:', c)
