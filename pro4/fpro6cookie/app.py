...
# render_template_string: 문자열로 작성한 Jinja 템플릿을 렌더링해 html로 반환
# make_response: 응답개체를 직접만들어..
# redirect: 다른 url로 이동
# url_for: 라우트함수 이름으로 url을 안전하게 생성하는 함수

app = Flask(__name__)

# 쿠키는 브라우저에 저장되는 작은 키-값 데이터이고, 서버가 클라이언트와 연결 상태를
# 유지하는 것처럼 할 수있다. 
...
# - 없으면 none을 반환한다.(첫 방문, 쿠키 만료, 쿠키 삭제된 경우)
...
def delete_cookie():
    # 쿠키 삭제 후에는 home으로 이동하기 위한 redirect 응답 만듦
