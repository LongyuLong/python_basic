...
class Node :
    def __init__(self, key):
        ...
        self.left = None    # 왼쪽 자식 노드 (key보다 더 작은 값들이 저장)
        self.right = None   # 오른쪽 자식 노드(key보다 더 큰 값들이 저장)
...
def insert(root, key) :
    ...
    if key < root.key :     # 넣은 값이 현재 노드보다 작으면
        root.left = insert(root.left, key)     #왼쪽 서브트리에 재귀적으로 삽입
    else :
        root.right = insert(root.right, key)    # 오른쪽 서브트리에 재귀적으로 삽입
...
root = None
for v in values :
    root = insert(root, v)  # BST에 삽입하고 루트(최상단)을 갱신
