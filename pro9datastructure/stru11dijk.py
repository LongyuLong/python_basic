# Dijkstra 알고리즘
# 가중치가 있는 그래프에서 한 시작점으로부터 모든 정점까지의 최단거리를 구하는 알고리즘
# 조건: 간선 가중치는 음수가 없어야하고, 그래프에는 여러 경로가 있을 수 있다.
...
def dijkstraFunc(graph, start):
    dist = {node:float('inf') for node in graph}    # 모든 노드까지의 최단 거리를 무한대
    dist[start] = 0     # 'A' = 0
    print(dist)
    ...
    while pq:           # 처리할 노드가 남아있는 동안
        ...
        if cur_dist > dist[u]:  # 더 짧은 경로가 있으면 현재 노드는 무시(중복방지?)
            continue

        for v, w in graph[u]:           # 현재노드 u와 연결된 이웃 노드 v 탐색
            new_dist = cur_dist + w     # u를 거쳐 v로 가는 새 거리 계산
            if new_dist < dist[v]:
                dist[v] = new_dist      # 최단 거리 저장소(최단 거리 갱신)
                heapq.heappush(pq, (new_dist, v))   # 최단거리로 큐(Min Heap)에 삽입
    return dist
