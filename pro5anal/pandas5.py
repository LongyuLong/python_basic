...
# skip_blank_lines : 칼럼명, 데이터 앞의 공백을 제거
print(df)
print()
...
print()
df = pd.read_csv('https://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/ex3.txt', sep='\s+')          # 공백 단위로 구분
print(df)
...
print()
df = pd.read_table('https://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/ex3.txt', sep='\s+', skiprows=1)
print(df)


print()
df = pd.read_fwf('https://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/data_fwt.txt', header=None, width=(10,3,5),names = ('date','name','price'),encoding='utf8')
print(df)
...
print()
print('\nchunck : 대량의 데이터를 부분씩 메모리로 읽어 처리')

# 대용량 자료 로딩시 초과 오류 발생 방지 : 메모리를 절약
# 스트리밍 방식(일부만 순차적 처리)으로 읽음
# 분산 처리의 효과
...
for i ,chunk in enumerate(pd.read_csv(csv_fname, chunksize = chunk_size)):                         # 데이터에 대한 인덱스를 얻을 수 있다.
    start_chunck = time.time()
    # 청크 처리 중 첫번째 학생 정보 출력
    first_student = chunk.iloc[0]
    print(
    f"Chunk {i + 1} 첫번째 학생 : ID = {first_student['id']}, 이름 = {first_student['name']}, "
    ...
    end_chunk = time.time()
    elapsed = end_chunk - start_chunck
...
print(f'score1 총합 : {total_score2}, 평균 : {average_chunk2:3f}')

print(f'전체 한번에 처리 시간 : , {time_all:7f}')
