...
aa = soup.select_one("div#hello > a")       # 표현이 새롭네
...
result = soup.select("#mw-content-text p")      # select 내부 설명필요

# print(result)   # 잘 가져오는건 확인 완료
for s in result:
...
df = pd.DataFrame({"싱품명":names,"가격":prices})
print(df.head(3))
print(f"가격 평균: {df['가격'].mean():.2f}")
print(f"가격 표준편차: {df['가격'].std():.2f}")

cv = df['가격'].std() / df["가격"].mean() * 100
print(f"가격 변동게수(CV): {cv:.2f}%")
# 해석: 가격 변동계수(CV): 28.31%이므로 평균 대비 적당히 퍼져있는 편
