...
print(root[0][0].tag)           # root 요소의 0번째의 0번째 요소명(노드명) 얻기. 다소 무식
...
print(root)     #출력결과: <Element '{current}current' at 0x0000021F7B103A60>

# {current} namespace 제거
for elem in root.iter():
    if '}' in elem.tag:
        elem.tag = elem.tag.split('}',1)[1]     # } 를 기준으로 잘라 태그명
# {current}weather -> weather 변경 완
...
for local in weather.findall("local"):
    ...
    ta = local.get('ta')                # local 요소(element) ta 속성(attribute)
    print(f"{name}지역 온도는 {ta}도")